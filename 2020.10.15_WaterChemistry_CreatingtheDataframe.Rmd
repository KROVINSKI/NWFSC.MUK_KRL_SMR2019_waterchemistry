...
spec.insitu <- read.csv(file = "2020.10.26_SpecData.csv", stringsAsFactors = FALSE)
...
```{r 10.2 Filtering and Modifying Data for insitu measurements}
...
#### 10.2a Bringing Day and Night Datasets Together
```{r}
...
# #DOinsitu.3 <- rbind(d.DO.insitu, n.DO.insitu)
...
#### 10.2b SPEC insitu data 
```{r 10.2b SPEC insitu data }
...
# spec.insitu$ObservationDate <- as.Date(spec.insitu$Date)


signif(spec.insitu$time, digits = 6)




spec.insitu <- spec.insitu %>% mutate(newtime=mdy_hms(paste0(date, "_", seconds_to_period(time*seconds.in.a.day)))) 

# signif(spec.insitu$period, digits = 4)
# 
# spec.insitu$newtime <- ""
# 
# seconds_to_period(spec.insitu$time*seconds.in.a.day)


# spec.insitu <- spec.insitu %>% mutate(rawtime=seconds_to_period(time*seconds.in.a.day))
# spec.insitu <- spec.insitu %>% mutate(rawtime=format(as.POSIXct(time*seconds.in.a.day, origin = spec.insitu$date), "%H:%M:%s"))
spec.insitu$rawtime <- format(as.POSIXct(spec.insitu$time*seconds.in.a.day, tz = "America/Los_Angeles", origin = spec.insitu$date), "%H:%M:%s")
spec.insitu <-  spec.insitu %>% mutate(newertime=mdy_hms(paste0(date, "_", rawtime)))

spec.insitu$ObservationTime <- format(as.POSIXct(dn.DO.insitu$dateTime) ,format = "%H:%M:%S")



# 
# spec.insitu$newtime <- mdy_hms(paste0)
...
spec.insitu$ObservationTime <- spec.insitu$time*86400

partialtime <- spec.insitu %>% mutate(period=seconds_to_period(ObservationTime)) %>% mutate(time_of_day=sprintf("%02i:%02i", hour(period), minute(period)))

partialtime [, time_of_day := sprintf("%02i:%02i", hour(period), minute(period))]
# partialtime <- spec.insitu[, period:= seconds_to_period(ObservationTime)]    

#spec.insitu$ObservationTime <- format(as.POSIXct(spec.insitu() + 0.8541667), "%H:%M", tz="UTC")
spec.insitu$ObservationTime <- format(as.POSIXct(spec.insitu$time, "%H:%M", tz="UTC"))
                                      
# spec.insitu$ObservationTime <- strftime(spec.insitu, format = "%STR 32460", tz = "", usetz = FALSE,                                   strftime(32460, format = "%S")
#                                         
                                        
spec.insitu$ObservationTime <- strftime(strftime(32460, format = "%S"))
# # create sample data
