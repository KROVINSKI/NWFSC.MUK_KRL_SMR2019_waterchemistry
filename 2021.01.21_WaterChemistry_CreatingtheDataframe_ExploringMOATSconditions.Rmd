---
title: "Creating the Water Chemistry Dataframe & MOATS Conditions"
output:
 html_document:
    df_print: paged
    code_folding: hide
...
![MOATS Lab, 13unit assembly](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL_SMR2019_waterchemistry/13MOATs.png)


### 1.2 Subsampling
For speed and ease of computations the dataframe was extremely subsampled. 
...
spec.insitu <- read.csv(file = "2020.10.26a_SpecData.csv", stringsAsFactors = FALSE)
...
Krill room, MOATS Lab was keep in low lights conditions throughout the experiment
Day and night conditions were governed by flow and no-flow periods. 
Flow was controlled with a solenoid valve.


![Solenoid Valve, Upstream of Animal Box](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL_SMR2019_waterchemistry/waterchem_system_solenoid_picture1.png)
...
PreSens optical oxygen Fitbox4 sensor and oxygen sensor spots (SP-PSt3-NAU-D5-YOP)
...
write.csv(H2Ochemdf, "2020.11.22_H2Ochemdf.csv")
...
write.csv(Cdml.daynight.aTemp.summary, "2020.11.22_Cdml_daynight_aTemp_summary.csv")
...
# filteredSPECFrame$treatment <- factor(filteredSPECFrame$treatment)
...
write.csv(H2Ochemdf.daynight.actualDO.summary, "2020.11.22_H2Ochemdf_daynight_actualDO_summary.csv")
...
write.csv(H2Ochemdf.daynight.pH.summary, "2020.11.22_H2Ochemdf_daynight_pH_summary.csv")
...
DOplot12.3 <- ggplot(H2Ochemdf, aes(treatment, actualDOmg)) +
...
#### 12.x DO data Summarized
```{r 12.x DO Day/Night insitu Summary}

dim(dn.DO.insitu)

DO.insitu.daynight.summary <- dn.DO.insitu %>% group_by(treatment, period) %>%
  summarize(sd = sd(actualDOmg, na.rm = TRUE), 
            mean = mean(actualDOmg, na.rm = TRUE), 
            median = median(actualDOmg, na.rm = TRUE),
            IQR = IQR(actualDOmg, na.rm = TRUE),
...
write.csv(DO.insitu.daynight.summary, "2020.11.22_DO.insitu.daynight.summary.csv")
```
...
pH conditions in each replicate were monitored via spectrophotometry using an Ocean Optics USB 230 2000+ Fiber Optic Spectometer and a 5mM solution of m-cresol purple indicator dye (Sigma Aldrich). 



```{r 13.1 Water Chemistry - pH Summary Table Plots}
H2Ochemplot1 <- ggplot(H2Ochemdf, aes(treatment, pH)) +
            geom_jitter(color = "grey") +
            geom_boxplot(notch = TRUE, outlier.shape = NA, colour = "green") +
...
```{r 13.1 Water Chemistry - pH Summary Table Plots Summary Statistics}

boxplot.stats(H20chemplot1)
...
#### 13.1a SPEC data Summarized
```{r 13.1a pH Day/Night insitu Summary}
...
write.csv(pH.insitu.daynight.pH.summary, "2020.11.22_pH.insitu.daynight.pH.summary.csv")
```
...
H2Ochemplot.pH <- ggplot(H2Ochemdf, aes(treatment, pH)) +
            geom_jitter(color = "grey") +
            geom_boxplot(notch = TRUE, outlier.shape = NA, colour = "green") +
            geom_point(data = H2Ochemdf.daynight.pH.summary, aes(x=treatment, y=mean), size=5, color = "purple") +
            geom_errorbar(data =H2Ochemdf.daynight.pH.summary,
...
```{r}
#setting up boxplot statitics 
#boxplot.stats(H2Ochemplot.pH)
